...
export type DeletionCertificateContent = {
  clientName: string;
  subjectId: string;
...
export async function buildDeletionCertificatePdf(
  content: DeletionCertificateContent,
): Promise<Uint8Array> {
  const doc = await PDFDocument.create();
  const regular = await doc.embedFont(StandardFonts.Helvetica);
  const bold = await doc.embedFont(StandardFonts.HelveticaBold);
  ...
  draw("BioVault Sentinel — DPDPA Deletion Confirmation Certificate", { bold: true, size: 16 });
  y -= 8;
  draw(`Client: ${content.clientName}`);
  draw(`Subject ID: ${content.subjectId}`);
  draw(`Erasure Request: ${content.erasureRequestId}`);
  draw(`Completed At: ${content.completedAt}`);
  draw(`Degauss Method: ${content.degaussMethod}`);
  draw(`Technician: ${content.technicianEmail}`);
  y -= 8;
  draw("Permanently Deleted Files", { bold: true });
  for (const name of content.filenames) {
    draw(`• ${name}`);
  }
  y -= 12;
  draw("Digital Signature", { bold: true });
  ...
  y -= 12;
  draw(
    "This certificate confirms permanent erasure of listed personal data per DPDPA 2023 Right to Erasure.",
    { size: 9, color: rgb(0.35, 0.35, 0.35) },
  );
